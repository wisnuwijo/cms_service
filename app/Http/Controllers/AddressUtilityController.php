...
class AddressUtilityController extends Controller
{
    ...
    public function sendSelectCourierMessage($uuid, $pendingTrx, $address, $couriers)
    {
    ...
    public function saveAddress(Request $req)
    {
        ...
        // if origin is not my account add integration with transaction
        if ($req->origin != "myaccount") {
            // prevent duplication
            ...
            // send wa message on success insert to tbl
            $saveAddress = Address::insert([
                "id" => Str::uuid(),
                "pending_transaction_id" => $req->pending_transaction_id,
                "client_id" => $req->client_id,
            ...
            if ($saveAddress) {
                $this->sendAddAddressConfirmMsg($uuid, $req->full_address, $req->receiver_phone_number, "OK");
            } else {
                $this->sendAddAddressConfirmMsg($uuid, $req->full_address, $req->receiver_phone_number, "FAILED");
            }
        }
    ...
    public function sendAddAddressConfirmMsg($uuid, $full_address, $to, $status)
    {
        $logPrefix = now() .' '. $uuid . ' sendAddAddressConfirmMsg() ';
        Log::info($logPrefix . "Send whatsapp message to client");
        ...
        try {
            ...
            Log::info(now() ." ". $uuid . " sendAddAddressConfirmMsg() Send request to whatsapp");
            Log::info(now() ." ". $uuid . " sendAddAddressConfirmMsg() Endpoint: " . $whatsappEndpoint);
            Log::info(now() ." ". $uuid . " sendAddAddressConfirmMsg() Payload: " . json_encode($payload));
            $response = $client->request('POST', $whatsappEndpoint, $payload);

            $content = json_decode($response->getBody()->getContents());
            Log::info(now() ." ". $uuid . " sendAddAddressConfirmMsg() Response: " . json_encode($content));
        } catch (ClientException $e) {
            $response = Psr7\Message::toString($e->getResponse());
            Log::info(now() ." ". $uuid . " sendAddAddressConfirmMsg() Error: " . $response);
        }
